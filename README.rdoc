...
Note that, unlike most other Unix tools, the *main arguments come first*,
and then the flags appear *with possibly several arguments*.  Consider:
...
For content searches, I recommend using mairix:
  ...
  % coma read $(mairix -r SEARCH TERMS)
...
== Other commands
  ...
  % coma
  Subcommands: att folders fwd inc mail mark next prev read repl scan show usage
...
Working with sequences: Sequences are refered to in the query language
as "%sequencename".  By default, you operate on the sequence
"%default" (or $COMASEQ if set).  You can list sequences with
"coma seq":
...
be deleted with "coma seq -clear sequencename...".  You can add mails
...
I accept no feature requests at this point; coma will become *my*
perfect mail reader.  But you are welcome to fork and patch, and maybe
in a few months, we can figure out what's good and what not, and then
merge accordingly.
